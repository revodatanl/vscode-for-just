...
// --- Stage renderers (produce HTML fragments for each stage container) ---

const renderStageLoading = (label: string): string =>
  `<div class="stage-loading"><span class="spinner-inline"></span> ${label}</div>`;

const renderStageError = (message: string, stage: string): string =>
...
const renderJustfileOk = (path: string, imports: string[]): string => {
  const importsHtml =
    imports.length > 0
  ...
  return (
    `<div class="stage-row">` +
    `<span class="justfile-path">${escapeHtml(path)}</span> ` +
    `${renderOpenFileBtn(path)}` +
...
const renderRecipesOk = (recipes: RecipeParsed[]): string => {
  ...
  const statsHtml =
    ...
    `<span class="recipe-stats">${totalRecipes} recipe${totalRecipes !== 1 ? 's' : ''}${totalGroups > 0 ? ` &middot; ${totalGroups} group${totalGroups !== 1 ? 's' : ''}` : ''}</span> ` +
...
export const refreshCli = async (
  ...
  post(postFn, 'cli', 'loading', renderStageLoading('Checking just CLI&hellip;'));
  ...
  if (version) {
    ...
    const err = 'just CLI not found on PATH';
    state.cli = { status: 'error', error: err };
...
export const refreshJustfile = async (
  ...
  try {
    ...
    const imports = await getJustfileImports(path);
    if (seq !== seqJustfile) return;

    state.justfile = { status: 'ok', path, imports };
    post(postFn, 'justfile', 'ok', renderJustfileOk(path, imports));
  } catch (error) {
    if (seq !== seqJustfile) return;

    const msg = errorMessage(error);
    state.justfile = { status: 'error', error: msg };
    post(postFn, 'justfile', 'error', renderStageError(msg, 'justfile'));
  }
};
...
export const refreshRecipes = async (
  ...
  try {
    ...
    post(postFn, 'recipes', 'error', renderStageError(msg, 'recipes'));
  }
};
...
export const sendCachedState = (state: WebviewState, postFn: PostFn): void => {
  ...
  if (state.justfile.status === 'ok' && state.justfile.path) {
    ...
    post(
      ...
      'justfile',
      'error',
      renderStageError(state.justfile.error, 'justfile'),
    );
  }
...
export const refreshNeeded = async (
  ...
  // Run cli + justfile checks in parallel (only for stages that need it)
  const tasks: Promise<void>[] = [];

  if (state.cli.status === 'idle' || state.cli.status === 'error') {
    tasks.push(refreshCli(state, postFn));
  }
  if (state.justfile.status === 'idle' || state.justfile.status === 'error') {
    tasks.push(refreshJustfile(state, postFn));
  }
